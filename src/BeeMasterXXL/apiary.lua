...
--载入蜂箱数据
local apiaryList, worldAccelerator_tier, worldAccelerator_mode = {}, 0, "moving"
do
    ...
    worldAccelerator_mode = config.worldAccelerator_mode
    if worldAccelerator_mode == nil then
        worldAccelerator_mode = "moving"
    end
    if worldAccelerator_mode ~= "moving" and worldAccelerator_mode ~= "fixed" then
        error(string.format(
            "错误的worldAccelerator_mode配置：%s（仅支持moving或fixed）",
            tostring(worldAccelerator_mode)
        ))
    end
end
...
local function setupApiary(location, damage, targetFoundation)
    ...
    --moving模式沿用原版拆放逻辑；fixed模式完全不接触手动放置的四台世界加速器。
    if worldAccelerator_mode == "moving" then
