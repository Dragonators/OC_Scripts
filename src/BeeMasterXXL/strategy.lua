...
function M.mutate(princessSlot, droneSlot, targetSpecies, mutation)--单步突变
    ...
    --2.执行突变(自动突变分支)
        ...
        local function nextGeneration(droneSlot)--追踪公主蜂
            local currentPrincessSlot = princessSlot
            princessSlot = princessTracker.nextGeneration(currentPrincessSlot, function()
                device.nextGeneration(currentPrincessSlot, droneSlot, mutation)
            end, "错误的调用strategy.mutate().nextGeneration，突变过程中")
        end
...
function M.purify(princessSlot, droneSlot, targetGenes, assistantDroneSlot, labelSuffix)--纯化
    ...
    local function nextGeneration(droneSlot)--追踪公主蜂
        local currentPrincessSlot = princessSlot
        princessSlot = princessTracker.nextGeneration(currentPrincessSlot, function()
            device.nextGeneration(currentPrincessSlot, droneSlot)
        end, "错误的调用strategy.purify().nextGeneration，提纯过程中")
    end
...
function M.breedDrones(princessSlot, droneSlot, targetAmount)--繁殖雄蜂
    ...
    while robot.count(droneSlot) < targetAmount do
        ...
        princessSlot = princessTracker.nextGeneration(currentPrincessSlot, function()
            device.nextGeneration(currentPrincessSlot, droneSlot)
        end, "错误的调用strategy.breedDrones().nextGeneration，繁殖过程中")
        droneSlot = nil
        for _,slot in pairs(bot.getItemsWithLabel(bot.inventoryLabel)) do
            if bot.inventory[slot].type == "beeDrone" then
...
function M.newSpecies(species, mutation)--突变新品种并优化基因
    --校验输入
    local function getParentTags()
        local parent1 = mutation.parents[1]
        local parent2 = mutation.parents[2]
        local tag1 = beeData.getDroneTag(parent1)
        local tag2
        if parent2 == parent1 then
            tag2 = tag1
        else
            tag2 = beeData.getDroneTag(parent2)
        end
        if not tag1 or not tag2 then
            local missing = {}
            if not tag1 then
                table.insert(missing, parent1)
            end
            if not tag2 and parent2 ~= parent1 then
                table.insert(missing, parent2)
            end
            error(string.format(
                "突变%s：缺少同时携带目标品种和原生效果的亲本雄蜂：%s",
                mutation.name,
    ...
    local allele1Tag, allele2Tag = getParentTags()
